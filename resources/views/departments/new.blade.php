  ...
  <div class="md:w-[90vw] mt-5 lg:w-2/3 w-full px-6 py-6 mx-auto">
    ...
    <div class="card bg-base-100 shadow-xl">
      <div class="p-6 flex pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
        <h6 class="font-bold text-lg flex-auto">Add Department</h6>
      </div>
      <div class="flex-auto p-5">
        <form role="form" method="POST" class="p-4" action="{{ route('departments.store') }}">
            @csrf
            @method('POST')
            <div class="mb-4 px-3">
            ...
            <div class="mb-4 px-3">
                <label class="form-control w-full">
                  <div class="label">
                    ...
                    <select class="select select-bordered w-full" name="visibility_telemetry">
                        @foreach(["public", "private"] as $visibility_telemetry)
                            <option value="{{$visibility_telemetry}}"
                                @selected(old('visibility_telemetry') == $visibility_telemetry)
